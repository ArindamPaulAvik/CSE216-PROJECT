...
import { FiUser, FiMail, FiCalendar, FiCamera, FiSave, FiArrowLeft, FiHeart, FiGrid, FiInfo, FiX, FiTrash2, FiStar, FiEdit3, FiEyeOff } from 'react-icons/fi';
...
function UserProfileView() {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (response.data.profile?.profilePicture) {
        setImagePreview(`http://localhost:5000/images/user/${response.data.profile.profilePicture}`);
      }
      ...
      if (error.response) {
        if (error.response.status === 401) {
          ...
          setError(error.response.data?.error || `Error ${error.response.status}: Failed to load profile`);
        }
  ...
  const fetchFavorites = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/profile/${userId}/favorites`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchUserComments = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/profile/${userId}/comments`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchUserRatings = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/profile/${userId}/ratings`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleTabSwitch = (tab) => {
    ...
    // Clear errors based on which tab we're switching to
    if (tab === 'details') {
      setFavoritesError('');
      setCommentsError('');
      setRatingsError('');
  ...
  const { profile, canViewRatings, isOwnProfile } = profileData;
          ...
          <div>
            ...
            <div className="profile-picture-section">
              ...
              <div className="profile-picture-info">
                ...
                <p>Member since {new Date(profile.createdAt || Date.now()).getFullYear()}</p>
              </div>
            </div>
            ...
            <div className="profile-nav">
              ...
              <button
                className={`nav-tab ${activeTab === 'ratings' ? 'active' : ''} ${!canViewRatings ? 'disabled' : ''}`}
                onClick={(e) => {
                  e.preventDefault();
                  if (canViewRatings) {
              ...
              <div className="tab-content">
                ...
                {loadingFavorites ? (
                  ...
                  <div className="favorites-empty">
                    ...
                    <p>Shows added to favorites will appear here</p>
                  </div>
                ) : (
                  <div className="favorites-list">
                    <div className="favorites-header">
                      <h3>Favorite Shows ({favorites.length})</h3>
                    </div>
              ...
              <div className="tab-content">
                ...
                {loadingComments ? (
                  ...
                  <div className="comments-empty">
                    ...
                    <p>Comments made on shows will appear here</p>
                  </div>
                ) : (
              ...
              <div className="tab-content">
                {!canViewRatings ? (
                  <div className="privacy-notice">
                    <FiEyeOff size={48} className="empty-icon" />
                    <h3>Ratings Private</h3>
                  ...
                  <>
                    ...
                    {loadingRatings ? (
                      ...
                      <div className="ratings-empty">
                        ...
                        <p>Episodes rated will appear here</p>
                      </div>
                    ) : (
